...
export function createFinalDomainHTML(domainObj, status, id) {
  ...
  let buttonHTML;
  if (status === "available") {
    buttonHTML = `<a href="${regLink}" target="_blank" rel="noopener" class="btn btn-sm btn-success action-btn">Available <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg></a>`;
  } else if (status === "unknown") {
    buttonHTML = `<button class="btn btn-sm btn-outline-warning action-btn" data-action="recheck" data-id="${id}">⚠️ Unknown · Retry</button>`;
  } else {
    buttonHTML = `<button class="btn btn-sm btn-secondary action-btn" disabled>Taken</button>`;
  }
